...
import getUser from '@salesforce/apex/RetrieveMetadata.getUser';
const ACOLS = [
...
export default class UserDependencies extends LightningElement {
    ...
    @track lst = [];
    @track test = false;
    @wire(getUser) nData({ data, error }) {
        if (data) {
            this.uData = data;
            this.uData.forEach(element => {
                this.usData = [...this.usData, { Id: element.Id, Name: element.Name, WorkspaceId: element.WorkspaceId, Username: element.Username, LastName: element.LastName, FirstName: element.FirstName }];
            });
        } else if (error) {
            console.log("Error in getApexClasses>>" + JSON.stringify(error));
        }
        if (this.usData.length) {
